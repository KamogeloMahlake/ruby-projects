...
class ComputerPlayer
  ...
  def codebreaker(previous_guess, feedback)
    ...
    previous_guess.each do |item|
      next if used_numbers.include?(item)

      used_numbers.unshift(item)
    end

    @numbers.each do |item|
      @numbers.delete(item) if used_numbers.include?(item)
    end
    secret_code = []
    while @previous_guess.include?(secret_code)
      ...
      when 1
        ...
        secret_code.unshift(previous_guess[0])
      when 2
        secret_code = Array.new(2, @numbers.sample)
        secret_code.unshift(previous_guess[0])
        secret_code.unshift(previous_guess[1])

      when 3
        secret_code = Array.new(1, @numbers.sample)
        secret_code.unshift(previous_guess[0])
        secret_code.unshift(previous_guess[1])
        secret_code.unshift(previous_guess[2])
      when 4
